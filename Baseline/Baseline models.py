...
import random
from tensorflow.keras.layers import Conv1D, Conv2D, Permute , MaxPooling1D, Dot, MaxPooling2D, TimeDistributed, AveragePooling2D, Lambda, Softmax, Dense, Flatten, Embedding, Bidirectional, LSTM, Dropout, Input, Reshape, concatenate, dot, Multiply, RepeatVector, Activation
...
def model_architecture_CNN():  
  ...
  r_final = concatenate([r_final] , axis=-1) #3,x+y
  ...
  model = Model(inputs=[paper_input, rev_input] , outputs=[classification_out,rec_out])
  model.compile(optimizer = "adam", loss = [BinaryCrossentropy, root_mean_squared_error] ,metrics =["accuracy","mse"])

  cls_model = Model(inputs=[paper_input, rev_input] , outputs=classification_out)
  cls_model.compile(optimizer = "adam", loss = BinaryCrossentropy ,metrics =["accuracy"])

  rec_model = Model(inputs=[paper_input, rev_input] , outputs=rec_out)
...
# For regression
trLen = len(yc)
_,clm,_ = model_architecture_CNN()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
...
trLen = len(yc)
clm,_,_ = model_architecture_CNN()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
...
for i in range(10):
  print("Epoch:",i)
  clm.fit(train_inputs, yc, batch_size = 32, shuffle=False,epochs = 1,verbose=1)
  tmp = clm.predict(test_inputs)
...
mat = [[0,0],[0,0]]
for i in range(teLen):
	mat[int(tc[i])][int(tc_prd[i])] += 1
  
def plot_conf_mat(cm , xlen, ylen , title="confusion matrix", cmap = plt.cm.Blues):
	fig, ax = plot_confusion_matrix(conf_mat=cm, figsize=(8, 8) , cmap=cmap , colorbar=True , show_normed=True, show_absolute=False)
...
print(classification_report(tc,clean_tc_prd(tc_prd), digits=4))
...
def model_architecture_LSTM():  
  ...
  ##paper only - cnn
  SC = Reshape((paper_mx_sections,paper_embed_dim))(paper_embedded)
  sc = LSTM(256, return_sequences=True)(SC)
  sc_final = Flatten()(sc)
  sc_final = Dropout(dropout)(sc_final)
  ...
  R = Reshape((3,rev_mx_snts,rev_embed_dim))(rev_embedded)

  r = TimeDistributed(LSTM(1024, return_sequences=True))(R)
  r = TimeDistributed(Flatten())(r)
...
# For regression
trLen = len(yc)
_,clm,_ = model_architecture_LSTM()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
...
# ==> classification_model
trLen = len(yc)
clm,_,_ = model_architecture_LSTM()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
...
for i in range(10):
  print("Epoch:",i)
  clm.fit(train_inputs, yc, batch_size = 8, shuffle=False,epochs = 1,verbose=1)
  tmp = clm.predict(test_inputs)
  f1= get_f1(clean_tc_prd(tmp),tc)
  if f1>max_f1:
    max_f1 = f1
...
def model_architecture_CNN_Bi_LSTM():  
  #paper - part
  paper_input = Input(shape=(paper_mx_sections,))
  paper_embedded = Embedding(paper_num_tokens, paper_embed_dim, embeddings_initializer=tf.keras.initializers.Constant(paperEmbed), trainable=False)(paper_input)
  ...
  ##paper only - cnn
  SC = Reshape((paper_mx_sections,paper_embed_dim))(paper_embedded)
  p_a = Bidirectional(LSTM(20, return_sequences=True))(SC)
  p_a = MaxPooling1D(pool_size=2)(p_a)
  p_a = Conv1D(64, 768, activation='relu', padding='same')(p_a)
  sc_vector = Flatten()(p_a)

  # rev only - cnn--------------------------------------------------------------------------
  
  R = Reshape((3,rev_mx_snts,rev_embed_dim))(rev_embedded)
  r_a = TimeDistributed(Bidirectional(LSTM(20, return_sequences=True)))(R)
  r_a = TimeDistributed(MaxPooling1D(pool_size=2))(r_a)
  r_a = TimeDistributed(Conv1D(64, 768, activation='relu', padding='same'))(r_a)
  r_final = TimeDistributed(Flatten())(r_a)
  r_vector = Flatten()(r_a)
...
# For regression
trLen = len(yc)
_,clm,_ = model_architecture_CNN_Bi_LSTM()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
...
#==> classification_model
trLen = len(yc)
clm,_,_ = model_architecture_CNN_Bi_LSTM()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
...
def model_architecture_CNN_Bi_LSTM_Attention():  
  #paper - part
  paper_input = Input(shape=(paper_mx_sections,))
  paper_embedded = Embedding(paper_num_tokens, paper_embed_dim, embeddings_initializer=tf.keras.initializers.Constant(paperEmbed), trainable=False)(paper_input)
  ...
  #Attention on reviews 
  activations = TimeDistributed(LSTM(64, return_sequences=True))(rev_embedded)
  attention = TimeDistributed(Dense(1, activation='tanh'))(activations) 
  attention = TimeDistributed(Flatten())(attention)
  attention = TimeDistributed(Activation('softmax'))(attention)
  attention = TimeDistributed(RepeatVector(units))(attention)
  ...
  r_a = TimeDistributed(Conv1D(64, 768, activation='relu', padding='same'))(r_a)
  r_final = TimeDistributed(Flatten())(r_a)
  r_final = concatenate([r_final,activations] , axis=-1)
  r_vector = Flatten()(r_final)
...
# For regression
trLen = len(yc)
_,clm,_ = model_architecture_CNN_Bi_LSTM_Attention()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
...
# for 5.44 ==> classification_model
trLen = len(yc)
clm,_,_ = model_architecture_CNN_Bi_LSTM_Attention()
train_inputs = [xp, xr]
test_inputs = [tp, tr]
